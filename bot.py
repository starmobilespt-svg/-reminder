...
def get_db_connection():
    # SQLite ချိတ်ဆက်မှုကို ခေါ်ယူရန်
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    return conn

def init_db():
    # Database Table မရှိသေးလျှင် အသစ်တည်ဆောက်ရန်
...
@app.route('/')
def home():
    return "Bot is awake and running on Local Database!"
...
def send_welcome(message):
    ...
    btn_backup = InlineKeyboardButton("📥 Backup DB", callback_data="backup_db")
    btn_restore = InlineKeyboardButton("📤 Restore DB", callback_data="restore_db")
    
    markup.add(btn_add, btn_view)
    markup.add(btn_delete)
    markup.add(btn_backup, btn_restore)
    
    bot.send_message(message.chat.id, "👋 မင်္ဂလာပါ! Local Database သုံး Reminder Bot ပါ။\n\nအောက်ပါခလုတ်များကိုနှိပ်၍ အသုံးပြုနိုင်ပါသည်။", reply_markup=markup)
    ...
    # ----------------- Backup & Restore Features -----------------
        ...
        if os.path.exists(DB_FILE):
            with open(DB_FILE, "rb") as file:
                bot.send_document(chat_id, file, caption="📦 သင့်ရဲ့ Local Database (.db) ဖိုင်ပါ။\nဒေတာများ ပျက်သွားပါက ဤဖိုင်ကို အသုံးပြု၍ ပြန်လည် Restore လုပ်နိုင်ပါသည်။")
        else:
            bot.send_message(chat_id, "❌ Database ဖိုင် မရှိသေးပါ။")

    elif call.data == "restore_db":
        msg = bot.send_message(chat_id, "📤 Restore လုပ်ရန် သင့်ထံတွင် သိမ်းဆည်းထားသော **tasks.db** ဖိုင်ကို ဤနေရာသို့ ပေးပို့ပါ။")
...
def process_restore(message):
    ...
    if message.document:
        try:
            ...
            # Database ဖိုင်ဟောင်းနေရာတွင် အသစ်ကို အစားထိုးခြင်း
            with open(DB_FILE, 'wb') as new_file:
                new_file.write(downloaded_file)
                
            bot.send_message(chat_id, "✅ Database ကို အောင်မြင်စွာ Restore လုပ်ပြီးပါပြီ။")
        except Exception as e:
            bot.send_message(chat_id, f"❌ Restore လုပ်ရာတွင် အမှားအယွင်းဖြစ်ပေါ်ခဲ့ပါသည်။\nError: {e}")
    else:
        bot.send_message(chat_id, "❌ မှန်ကန်သော Database Document ဖိုင် မဟုတ်ပါ။")
...
if __name__ == "__main__":
    ...
    print("Bot is running with SQLite Local Database...")
